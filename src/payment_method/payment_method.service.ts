...
export class Payment_MethodService {
  constructor(@InjectModel(Payment_Method) private payment_methodRepository: typeof Payment_Method) {}
  ...
  findAll() {
    return this.payment_methodRepository.findAll();
  }

  async findOne(id: number) {
    const payment_method = await this.payment_methodRepository.findOne({ where: { id } });
    if (!payment_method) {
      throw new HttpException('Bunday payment_method topilmadi', HttpStatus.NOT_FOUND);
    }
  ...
  async update(id: number, updatePayment_MethodDto: UpdatePayment_MethodDto) {
    ...
    return this.payment_methodRepository.update(updatePayment_MethodDto, { where: { id } });
  }
