...
export class Delivery_MethodService {
  constructor(@InjectModel(Delivery_Method) private delivery_methodRepository: typeof Delivery_Method) {}
  ...
  findAll() {
    return this.delivery_methodRepository.findAll();
  }

  async findOne(id: number) {
    const delivery_method = await this.delivery_methodRepository.findOne({ where: { id } });
    if (!delivery_method) {
      throw new HttpException('Bunday delivery_method topilmadi', HttpStatus.NOT_FOUND);
    }
  ...
  async update(id: number, updateDelivery_MethodDto: UpdateDelivery_MethodDto) {
    ...
    return this.delivery_methodRepository.update(updateDelivery_MethodDto, { where: { id } });
  }
