...
export class Discount_CouponService {
  constructor(@InjectModel(Discount_Coupon) private discount_couponRepository: typeof Discount_Coupon) {}
  ...
  findAll() {
    return this.discount_couponRepository.findAll();
  }

  async findOne(id: number) {
    const discount_coupon = await this.discount_couponRepository.findOne({ where: { id } });
    if (!discount_coupon) {
      throw new HttpException('Bunday discount_coupon topilmadi', HttpStatus.NOT_FOUND);
    }
  ...
  async update(id: number, updateDiscount_CouponDto: UpdateDiscount_CouponDto) {
    ...
    return this.discount_couponRepository.update(updateDiscount_CouponDto, { where: { id } });
  }
